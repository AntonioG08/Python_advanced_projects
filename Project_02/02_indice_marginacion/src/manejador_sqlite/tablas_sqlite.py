...
class TablaSQLite:
    ...
    def __init__(self, ruta_archivo):
        ...
        self.crear_tabla()  # With this we instruct to create the table
    ...
    def ejecutar(self, sentencias: str, *, valores: list[tuple] = None):
        ...
        la sentencia. Esto únicamente funciona con sentencias que meten o modifican
        valores.
        """
        # Creamos la conexión con el archivo
        conn = sqlite3.connect(self.ruta_archivo)  # We stablish a connection with the previously created DataBase
        cursor = conn.cursor()
    ...
    def crear_tabla(self):
        ...
        self.ejecutar(sentencia)  # We order SQLite3 to execute the command that we passed
    ...
    def select(self, columnas: list[str] = None, *, where: dict = None, limit: int = None) -> tuple | None:
        ...
        especificadas en la lista 'columnas', si no se especifica nada entonces
    ...
    def select_distinct(self, columnas: list[str], *, where: dict = None):
        r"""
        Retorna una lista con las diferentes tuplas formadas por los valores de las columnas especificadas en
        `columnas`. Además, podemos imponer restricciones mediante el atributo `where` que mapea nombres de columnas
        a valores.
        """
        # Si no se especifican restricciones definimos unas vacías
        if not isinstance(where, dict):
    ...
    def delete(self, *, where: dict = None):
        ...
        Para especificar qué filas eliminar, utilizamos el argumento 'where'
